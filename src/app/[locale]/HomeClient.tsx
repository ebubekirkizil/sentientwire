...
export default function HomeClient({ dbArticles, locale }: { dbArticles: any[]; locale: string }) {
  ...
  useEffect(() => {
    if (locale !== 'en-US' && locale !== 'en-GB' && locale !== 'en-CA' && locale !== 'en') {
...
function NewsletterPopup() {
  ...
  useEffect(() => {
    ...
    const stored = localStorage.getItem("sw_newsletter_dismissed");
    if (stored === "true") {
  ...
  const handleClose = () => {
    setIsVisible(false);
    setHasDismissed(true);
    localStorage.setItem("sw_newsletter_dismissed", "true");
  };
  ...
  return (
    <div style={{
      ...
      padding: 24, background: "rgba(0,0,0,0.85)", backdropFilter: "blur(6px)",
      ...
      <div style={{
        ...
        <button 
          ...
          style={{
            ...
            fontSize: 24, cursor: "pointer", transition: "color 0.2s"
          }}
        ...
        <p style={{
          ...
          Join thousands of analysts and decision-makers. Get unfiltered geopolitical intelligence delivered directly to your inbox.
        </p>

        {status === "success" ? (
          <div style={{ padding: 16, background: "rgba(16,185,129,0.1)", border: "1px solid #10b981", borderRadius: 8, color: "#10b981", fontFamily: "JetBrains Mono, monospace", fontSize: 13, textAlign: "center" }}>
            ✓ Subscription confirmed. Intel secured.
          </div>
        ) : (
          <form onSubmit={handleSubscribe} style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            ...
            <button
              ...
              style={{
                ...
                border: "none", borderRadius: 8,
                fontFamily: "Space Grotesk, sans-serif", fontWeight: 700, fontSize: 15, textTransform: "uppercase",
                cursor: status === "loading" ? "wait" : "pointer",
                boxShadow: "0 0 20px rgba(6,182,212,0.4)", transition: "all 0.2s"
              }}
              onMouseEnter={(e) => (e.currentTarget.style.background = "#22d3ee")}
              onMouseLeave={(e) => (e.currentTarget.style.background = "var(--cyan)")}
            >
              {status === "loading" ? "Processing..." : "Subscribe for Intel"}
            </button>
            {status === "error" && (
              <p style={{ color: "#ef4444", fontSize: 12, fontFamily: "JetBrains Mono, monospace", marginTop: 4 }}>
