...
export default function AnalyticsDashboard() {
  ...
  if (loading) {
    return <div className="text-center font-mono text-[var(--cyan-bright)] animate-pulse py-20">{t('initializingTelemetry')}</div>;
  }

  if (!data) return <div className="text-red-500 font-mono text-center">{t('systemFailureNoTelemetry')}</div>;
  ...
  return (
    <div className="flex flex-col gap-8">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="bg-[var(--bg-primary)] border border-[var(--border-subtle)] p-6 rounded-lg flex flex-col items-center justify-center relative overflow-hidden">
          <div className="absolute top-4 right-4 flex items-center gap-2">
             ...
             <span className="font-mono text-[10px] text-[var(--text-muted)]">{t('live')}</span>
          </div>
          <h3 className="font-mono text-xs text-[var(--text-muted)] tracking-widest mb-2">{t('activeSessions')}</h3>
          ...
          <p className="font-mono text-[10px] text-[var(--text-muted)] mt-4 border-t border-[var(--border-subtle)] pt-4 w-full text-center">
            {t('totalToday')}{data.todayVisits || 0}
          </p>
        </div>

        {/* Top Articles Card */}
        <div className="bg-[var(--bg-primary)] border border-[var(--border-subtle)] p-6 rounded-lg">
          <h3 className="font-mono text-xs text-[var(--text-muted)] tracking-widest mb-4 border-b border-[var(--border-subtle)] pb-2">{t('topArticles')}</h3>
          <div className="flex flex-col gap-3">
            {data.topArticles?.length === 0 && <div className="text-[var(--text-muted)] text-sm font-mono">{t('noData')}</div>}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {/* Referrers */}
        <div className="bg-[var(--bg-primary)] border border-[var(--border-subtle)] p-6 rounded-lg">
          <h3 className="font-mono text-xs text-[var(--text-muted)] tracking-widest mb-4 border-b border-[var(--border-subtle)] pb-2">{t('trafficSources')}</h3>
          <div className="flex flex-col gap-3">
            {data.referrers?.length === 0 && <div className="text-[var(--text-muted)] text-sm font-mono">{t('noData')}</div>}
        ...
        {/* Countries */}
        <div className="bg-[var(--bg-primary)] border border-[var(--border-subtle)] p-6 rounded-lg">
          <h3 className="font-mono text-xs text-[var(--text-muted)] tracking-widest mb-4 border-b border-[var(--border-subtle)] pb-2">{t('geoDistribution')}</h3>
          <div className="flex flex-col gap-3">
            {data.countries?.length === 0 && <div className="text-[var(--text-muted)] text-sm font-mono">{t('noData')}</div>}
